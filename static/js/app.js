...
function renderLogList() {
    ...
    const currentListHtml = logData.map(log => {
        return /* HTML */ `
            <div class="log-item" onclick="toggleLogSelection('${log.id}')">
                <input type="checkbox" id="checkbox-${log.id}" ${selectedLogs.has(log.id) ? 'checked' : ''} onchange="event.stopPropagation(); toggleLogSelection('${log.id}')">
    ...
    if (listElement.innerHTML !== currentListHtml && currentListHtml !== '') {
        ...
        listElement.innerHTML = /* HTML */ `<div class="loading-spinner">No logs found</div>`;
    }
}
...
function prepareLogContent() {
    ...
    if (selectedLogs.size === 0) {
        filteredLogLines = [];
        containerElement.innerHTML = /* HTML */ `<div class="log-placeholder">Select a log file to view content...</div>`;
        return;
    }
    ...
    const containerWidth = containerElement.clientWidth > 0 ? containerElement.clientWidth - 40 : 800;
    const charWidth = 7.8;
...
function renderVirtualScroll(forceUpdate = false) {
    ...
    if (filteredLogLines.length === 0 && selectedLogs.size > 0) {
        containerElement.innerHTML = /* HTML */ `<div class="log-placeholder">No matching log lines found</div>`;
        return;
    }
    ...
    while (endIndex < filteredLogLines.length && filteredLogLines[endIndex].offset < scrollTop + containerHeight) {
        endIndex++;
    }
    endIndex = Math.min(filteredLogLines.length, endIndex + VISIBLE_ROW_BUFFER);

    if (!forceUpdate && Math.abs(startIndex - currentRenderStartIndex) < 5 && Math.abs(endIndex - currentRenderEndIndex) < 5) {
        return;
    }
    ...
    const totalHeight = filteredLogLines.length > 0 ? filteredLogLines[filteredLogLines.length - 1].offset + filteredLogLines[filteredLogLines.length - 1].height : 0;
    const offsetY = filteredLogLines.length > 0 ? filteredLogLines[startIndex].offset : 0;

    const visibleLines = filteredLogLines.slice(startIndex, endIndex);

    const linesHtml = visibleLines.map(item => {
        return /* HTML */ `
            <div class="log-line ${item.isMatch ? 'match' : ''}">
                <span style="color: var(--accent); opacity: 0.7; font-size: 0.8em; margin-right: 8px;">[${item.logName}]</span>${escapeHtml(item.line)}
            </div>
        `;
    }).join('');
    ...
    if (themeToggleElement) {
        themeToggleElement.addEventListener('click', () => {
            const toggleTheme = () => {
                ...
                if (imgIcon) {
                    imgIcon.src = isLight ? '/static/image/icons/moon.svg' : '/static/image/icons/light.svg';
                }
            };
        ...
        const imgIcon = themeToggleElement.querySelector('.theme-icon');
        if (imgIcon) {
            imgIcon.src = savedTheme === 'light' ? '/static/image/icons/moon.svg' : '/static/image/icons/light.svg';
        }
    }
}
